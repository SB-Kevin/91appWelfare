...
- **活動辦法／獎品**：含「運動基金」動態計算（參與人數 × 100 元；命中冠軍者平分，四捨五入至 50 進位）。
- **立即預測 CTA**：連到 Google 表單，連結可由 Sheet 即時更換。
- **國旗本地化**：32 國國旗改用打包在 `flags/` 的 SVG 圖檔（含英格蘭子旗），解決 Windows／Android 無法顯示 emoji 國旗（會退化成 ZA／CA 文字）的問題；本地缺檔時自動回退 CDN。
- **長國名自動縮字**：國名依字數自動縮小字級（如「波士尼亞與赫塞哥維納」），確保 OUT 標記與比分不擠、不破版。
- **手機版優化**：RWD 自適應。
...
| 檔案 | 說明 |
|---|---|
| `WorldCupPrediction.html` | 活動網頁本體（單檔，含 HTML／CSS／JS）。直接用瀏覽器開啟即可。 |
| `flags/` | 32 國國旗 SVG（含英格蘭 `gb-eng`）。**須與 `WorldCupPrediction.html` 放同一層**一起上傳；來源為 flag-icons（MIT）。 |
...
員工 ── Google 表單 ──▶ Google Sheet（predictions / teams / results / meta）
                                  │
                       Apps Script Web App
                       ├─ doGet：回傳 JSON ───────▶ WorldCupPrediction.html（fetch 顯示）
...
- ⚠️ **`score`／`pk` 兩欄請設為「純文字」格式**：否則 Google Sheet 會把 "1-1" 自動轉成日期。把整欄設純文字後，`importFromDrive` 寫入的值就會原樣保留，**不需要改動 Apps Script**。（repo 內的 `Code.gs` 另含選用的加強防護：寫入前強制文字、讀取時還原誤轉日期；沒有部署也沒關係，網頁端也會過濾非比分格式。）
...
3. 將 `/exec` 網址填入 `WorldCupPrediction.html` 的 `CONFIG.API_URL`。
4. `meta` 設 `demo_mode=FALSE`、填 `form_url`。
5. 把 `results` 分頁的 `score`／`pk` 兩欄設為「純文字」格式（避免比分被轉成日期）。
6. Apps Script 設 `importFromDrive` 每日 07:10 時間觸發器。
7. 確認 Claude 的 Google Drive 連接器與 Sheet 同一 Google 帳號；對排程按一次「Run now」預先授權。
8. 部署 `WorldCupPrediction.html` 到可公開存取的位置（或內部空間）；**務必連同 `flags/` 資料夾一起上傳，且與 HTML 放同一層**，國旗才會正確顯示。

---

## 九、限制與注意事項

- 球星資料涵蓋晉級的 32 隊（每隊代表球星，資料由 `players` 分頁驅動）；展示模式仍用內建 16 國範例。
- 賽程的對戰組合在小組賽結束前為官方「組別代號」占位，非真實隊名／國旗。
- 國旗以 `flags/` 內的 SVG 為主（不需連網）；僅在本地缺該面國旗時才回退外部 CDN。
- 自動寫回 Sheet 需「可寫入試算表的途徑」（Drive 交接觸發器、或外部 POST）；單靠 Claude 排程無法直接 POST。
- 網頁為公開頁面，JSON 內容（不含 email）任何人可見；勿放敏感資料。
- 球員照片與 API 讀取需連網（國旗已本地化，不需連網）。
...
**Q11. 比分變成日期（如出現「Jan 01」或整列破版）？**
Google Sheet 把 "1-1"、"2-3" 自動判讀成日期所致。解法：把 `results` 的 `score`／`pk` 兩欄設為「純文字」格式即可（**不需重新部署 Apps Script**）；已誤轉的儲存格改文字格式後重打一次。（repo 的 `Code.gs` 另有選用防護，網頁端也會過濾非比分格式，屬多一層保險。）

**Q12. 國旗顯示不出來，只看到 ZA／CA 等英文字？**
這是 Windows／Android 缺 emoji 國旗字型的退化顯示。最新版已改用 `flags/` 內的 SVG 圖檔。請確認上傳時 **`flags/` 資料夾與 `WorldCupPrediction.html` 放在同一層**；若整批國旗都不見，多半是路徑不對或漏傳 `flags/` 資料夾。
